...
def extract_content_with_images(soup):
    ...
    return str(content_div) # نعيده كـ HTML للحفاظ على الصور
...
def scrape_novel_from_url(url):
    ...
    # 2. استخراج روابط الفصول (محاولة عامة + مخصصة لكل موقع)
    chapter_links = []
    for a in soup.find_all('a', href=True):
        href = a['href'].lower()
        # روابط الفصول الشائعة في معظم المواقع
        if any(k in href for k in ['chapter', 'chap-', 'novel/', 'part/']): 
            chapter_links.append(a['href'])
    
    chapter_links = list(dict.fromkeys(chapter_links)) # إزالة التكرار مع الحفاظ على الترتيب
...
def convert_to_html(book_data, output_filename):
    ...
    for chap in book_data['chapters']:
        # هنا نحافظ على الـ HTML (الصور Base64 والجداول)
...
def convert_to_azw3(book_data, output_filename):
    # في EbookLib، أفضل طريقة هي بناء ملف EPUB عالي الجودة، 
    # ثم إعادة تسميته إلى .azw3 لأن أجهزة Kindle الحديثة تقرأ EPUB الأصلية بشكل ممتاز.
    return convert_to_epub(book_data, output_filename)

# ==========================================
# دوال Flask (الواجهة الخلفية)
# ==========================================
